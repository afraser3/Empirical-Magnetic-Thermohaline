...
The \textit{de facto} thermohaline mixing model \textbf{used in MESA (first implemented and described in \citealt{mesa2} \textcolor{red}{[AF to MJ: I fully approve of this change except I think it was first implemented and described in Matteo's 2010 paper. But if you're specifically trying to point out the first Paxton et al paper, then this is fine with me]}) is commonly referred to as the ``Kippenhahn model'' and} was originally derived by \citet{ulrich_1972} and \citet{kippenhahn_etal_1980}.
%Thermohaline mixing is treated as a diffusion process, with a diffusion coefficient
Using arguments based on dimensional grounds and assumptions about the shapes and motions of discrete fluid parcels, they \sout{predicted} \textbf{conceived of} \textcolor{red}{[AF to MJ: I prefer ``predicted" over ``conceived". What do you think of ``arrived at"? Or ``derived"?]} a mixing efficiency of the form
...
Importantly, they note that this model and their simulations predict significantly less mixing than the Kippenhahn model with $C_t \sim 10^2$ or $10^3$ does.
 %\textcolor{red}{[Now that we've removed it from the rest of the paper, should we drop the actual formula for the Traxler prescription? Or should we include it since it's still in MESA?]}
...
This prescription \textbf{was first implemented in MESA in XXX} and has since been used in \citep{bauer_bildsten_2019} and other works. %\textcolor{red}{[Meridith/Evan: do we put the MESA version where this was first implemented?]}.
...
%For fixed diffusivities and background gradients, this model reduces to Eq.~\eqref{eqn:brown_model} for sufficiently small $H_B$
...
Furthermore, the dependence of $\Dth/\kappa_\mu$ on $r$ is profoundly different than either of the hydrodynamic models, with $\Dth/\kappa_\mu$ increasing with $r$, including as the thermohaline instability approaches marginal stability as $r \to 1$.
...
In this \sout{paper} \textbf{analysis}, we will \sout{determine if} \textbf{explore the question of whether stellar} observations %\textbf{are sufficiently discerning to}
are capable of 
\sout{probing the differences} \textbf{discerning} 
between these \sout{different} models. 
%are not.
