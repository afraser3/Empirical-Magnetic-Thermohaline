...
We study four grids of stellar evolution simulations with different thermohaline mixing prescriptions. One grid employs the \citet{brown_etal_2013} prescription with the recommended \sout{coefficient of \textbf{[COEFFICIENT NAME = ]} 1}\red{\texttt{thermohaline\_coeff = 1}}, while the other three employ the \citet{kippenhahn_etal_1980} prescription with coefficients $\alpha_{\rm th} \in [0.1, 2, 700]$. 
...
\red{We take the thermohaline timescale to be the diffusive timescale associated with thermohaline mixing, $t_{\rm th} = d^2/D_{\rm th}$, where $d$ is the radial depth of the thermohaline zone.}
In the case where $\alpha_{\text{th}} = 0.1$, 
%the \sout{evolutionary timescale is much shorter than the thermohaline timescale} 
\red{$t_{\rm evol} \ll t_{\rm th}$}. In the case where $\alpha_{\text{th}} = 700$, 
%\sout{the thermohaline timescale is much shorter than the evolutionary timescale} 
\red{$t_{\rm evol} \gg t_{\rm th}$}. In the intermediate case ($\alpha_{\text{th}}= 2$), the timescales are comparable (this is likewise true for the assumptions made in the Brown model).
