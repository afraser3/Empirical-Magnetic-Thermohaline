...
We exclude the first 21 models in which the thermohaline zone spans at least 10 \textbf{mass shells} %bins 
...
We measure $\epsilon = |\langle \delta_m \rangle / \mathrm{max}(\Delta_m^j)|$; if $\epsilon < 5 \times 10^{-3}$, we %mark a model as ``good'' 
