...
with h5py.File('hdf5_lite.h5', 'r') as f:
    ...
    for lognum in sorted([int(k) for k in f['profiles'].keys()]):
        ...
        nu_rad = 4 * a_rad *  T**4 / (15 * c_light * opacity * rho**2)
        ln_lam = -17.9 + 1.5*np.log(T) - 0.5 * np.log(rho)
        ln_lam[T >= 4.2e5] = (-11.5 + np.log(T) - 0.5*np.log(rho))[T >= 4.2e5]
        nu_ie = 2.21e-15 * (T**(5/2) / rho) / ln_lam
        nu = nu_ie + nu_rad
        d = (kt * nu / brunt_N2_structure_term)**(1/4)
        B0 = 100 #gauss
        mu0 = 4*np.pi
        HB = B0**2 * d**2 / (rho * mu0 * kt**2)
...
#            good_bool *= (radius > radius_th.min() + 0.25*radial_extent)*(radius <= radius_th.max() - 0.25*radial_extent)
            ...
            avg_HB = np.trapz(HB[good_bool]*dV, x=radius_th)/np.trapz(dV, x=radius_th)
            ...
            if len(good_point) < 21:
                ...
                if avg_diff < 5e-3:
                    ...
                    good_point.append(False)
            log_str = 'age: {:.2e}, tau_mix: {:.2e}, HB = {:.2e} \ndr = {:.3e} cm, dm = {:.2e}, good = {}'.format(good_ages[-1], tau_mixs[-1], avg_HB, radial_extent, mass_extent, good_point[-1])
            ...
            if PLOT_MOVIE:
                ...
                ax.plot(mass_coord[good_mass], np.log10(HB[good_mass]), label='log_HB')
